...
## 16. Game (zMath 전용) — 게임·챌린지·캐릭터 일러스트레이션

- ChallengeCard: 챌린지 카드. 난이도 헤더 스트립 + 제목 + 기간 + 진행률 바 + CTA. level(easy/medium/hard), progress(0-100) (`components/challenge/ChallengeCard.jsx`)
- ConceptGameCard: 수학 개념 게임 카드. 카테고리 컬러 헤더 + PeepsAvatar + 난이도 뱃지 (`components/game/ConceptGameCard.jsx`)
- MathConceptIllustration: 수학 개념별 SVG 일러스트. conceptKey·category로 대응 일러스트 렌더링, size 지원 (`components/game/MathConceptIllustration.jsx`)
- PeepsAvatar: Open Peeps 스타일 캐릭터 일러스트. variant(curious/focused/celebrating/confident), size, backgroundColor (`components/game/PeepsAvatar.jsx`)
- ProfileAvatar: 사용자 프로필 아바타. PeepsAvatar 기반 원형 클립 + 테두리. variant, size (`components/game/ProfileAvatar.jsx`)
- ProfileCard: 사용자 프로필 카드 오버레이. MUI Dialog. 아바타·닉네임·마지막 접속·풀이 목록 테이블(썸네일+진행률+O/X). onGameClick으로 문제 상세 이동 (`components/game/ProfileCard.jsx`)
- FullBodyAvatar: PeepsAvatar(상반신) + 다리+신발 SVG 조합 전신 캐릭터. variant, size, pantColor, shoeColor (`components/game/FullBodyAvatar.jsx`)
